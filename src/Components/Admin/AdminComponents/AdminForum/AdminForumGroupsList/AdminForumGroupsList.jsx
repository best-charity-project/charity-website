...
class AdminForumGroupsList extends Component {
  ...
  render() {
    return (
      <div className="forum-list">
        ...
        <div
          key="admin-forum-modal"
          className={
            this.state.isModalWindowOpen ? "overlay" : "overlay hidden"
          }
  ...
  submit = () => {
    confirmAlert({
      title:
        this.state.checkedGroupsIds.length && this.state.checkedTopicsIds.length
          ? "Группы будут удалены вместе с темами, темы вместе с ответами"
          : this.state.checkedTopicsIds.length
          ? "Темы будут удалены вместе с ответами"
  ...
  deleteChosenRecords = records => {
    if (
      ...
      this.deleteTopics();
      this.deleteGroups();
    } else if (this.state.checkedGroupsIds.length) {
      this.deleteGroups();
    } else {
      this.deleteTopics();
    }
  ...
  deleteGroups = () => {
    axios({
      ...
      .then(result => {
        ...
        let newGroups = this.state.filteredGroups.filter(
          forumGroups => !~result.data.forumGroups.indexOf(forumGroups._id)
        );
        this.setState({
  ...
  deleteTopics = () => {
    axios({
      ...
      .then(result => {
        this.props.changeState();
        let newTopics = this.state.filteredTopics.filter(
          forumTopics => !~result.data.forumTopics.indexOf(forumTopics._id)
        );
        this.setState({
  ...
  closeModalWindow = e => {
    if (
      ...
      ~e.target.className.indexOf("close-window")
    ) {
      this.setState({
        isModalWindowOpen: false
      });
  ...
  deleteItem = item => {
    ...
    axios({
      ...
      .then(result => {
        this.props.changeState();
        let newGroups = this.state.filteredGroups.filter(
          item => item._id !== result.data.forumGroup._id
        );
        this.setState({
          filteredGroups: newGroups,
          groups: newGroups
  ...
  deleteTopic = item => {
    ...
    axios({
      ...
      .then(result => {
        this.props.changeState();
        let newTopics = this.state.filteredTopics.filter(
          item => item._id !== result.data.forumTopic._id
        );
        this.setState({
          filteredTopics: newTopics,
          topics: newTopics
