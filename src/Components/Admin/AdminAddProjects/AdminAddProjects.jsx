...
class AdminAddProjects extends Component {
  ...
  componentDidMount() {
    ...
    if (this.props.location.state) {
      let fullTextEditorState = EditorState.createWithContent(
        convertFromRaw(JSON.parse(this.props.location.state.detail.fullText))
      );
      this.setState({
  ...
  render() {
    let newValue = draftToHtml(
      convertToRaw(this.state.fullTextEditorState.getCurrentContent())
    )
      .replace(/<[^>]*>/g, "")
      .replace(/\r\n/g, "").length;
    return (
            ...
            <div className="admin-projects-text-container">
              ...
              <div className="admin-media-projects">
                <div className="admin-image">
                  ...
                  <div
                    className={
                      this.isCorrectArrayLimit()
                        ? "admin-button"
                        : "button-projects-dislable"
                    }
                  >
                    ...
                    <div className="image-array">
                      {this.state.mediaImageArray.map((link, index) => (
                        <div className="projects-gallery-container" key={index}>
                          <img
                            src={link.name}
                            className="projects-media-gallery"
                            alt=""
                          />
                          <Button
                            name={"button-admin admin-cancel"}
                            label={<span aria-hidden="true">&times;</span>}
                            clickHandler={event =>
                              this.deleteGalleryImage(event, index)
                            }
                          />
                        </div>
                      ))}
                  ...
                  <div className="input-video-container">
                    <div className="add-info-projects">
                      ...
                      <Button
                        ...
                        name={
                          this.isCorrectArrayLimit()
                            ? "admin-button admin-projects-media-buttons"
                            : "button-projects-dislable"
                        }
                      />
                    </div>
                        ...
                        {this.state.mediaVideoArray.map((link, index) => (
                          <li className="projects-video-container" key={index}>
                            ...
                            <Button
                              ...
                              label={<span aria-hidden="true">&times;</span>}
                              clickHandler={event =>
                                this.deleteGalleryVideo(event, index)
                              }
                            />
                          </li>
                        ))}
            ...
            <div className="text-projects">
              ...
              <div className="projects-textfield">
                ...
                <div className="admin-textarea-description">
                  <span>
                    Краткое описание не должно содержать более 1000 символов
                  </span>
                  <div>
                    <span>Количество оставшихся символов: </span>
                    <span readOnly className="admin-textarea-symbols">
  ...
  addContacts = e => {
    ...
    if (
      this.state.contacts &&
      /^\+375\s\(\d{2}\)\s\d{3}-\d{2}-\d{2}$/.test(this.state.contacts)
    ) {
  ...
  isCorrectArrayLimit = () => {
    return (
      this.state.mediaImageArray.length + this.state.mediaVideoArray.length < 4
    );
  };
